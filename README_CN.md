...
* [基于 acl 构建的实际产品](#基于-acl-构建的实际产品)
    * [webcool（文酷）](#webcool文酷)
...
[webcool](https://webcool.cn/) 是一套基于 acl 构建的个人网盘与私有文件管理产品，支持在自己的服务器、工作站或家庭网络中私有部署。它使用 C++ 实现服务端，将 acl 的网络通信、HTTP 服务、并发处理和服务器框架能力应用到真实的跨平台产品中。

webcool 的主要能力包括：

- 远程虚拟磁盘与服务器本地磁盘管理；
- 文件上传、下载、复制、移动、标签、加锁、回收站和备份迁移；
- 图片、视频、音频、PDF、Office、Markdown 和 XMind 等格式预览；
- 浏览器内视频剪辑、音轨与字幕处理、视频转码及画质增强；
- AI 图片超分、去模糊、去红眼、去水印和人脸修复；
- iOS 与 Android 客户端、相册备份、离线缓存和视频投屏；
- Windows、macOS 与 Linux 跨平台部署。

在服务端实现中，webcool 使用 acl 提供的高性能网络与 HTTP 基础设施、协程并发模型、服务管理能力及数据库封装，将文件管理、媒体处理和多终端访问整合为一套可长期运行的实际应用。

- 产品官网：[https://webcool.cn/](https://webcool.cn/)
- 在线演示：[https://demo.webcool.cn/](https://demo.webcool.cn/)
- 软件下载：[https://webcool.cn/download/](https://webcool.cn/download/)
